...
class BigNum(object):
  ...
  def slow_mul(self, other):
    ...
    result = BigNum.zero(in_digits * 2)
    for i in xrange(1,len(self.d)):
      carry = Byte.zero()
      for j in xrange(1,len(other.d)):
        digit = self.d[i]*other.d[j] + result.d[i+j-1].word() + carry.word()
        result[i+j-1], carry = digit.lsb(), digit.msb()
      result[i + len(self.d)] = carry
    return result
  ...
  def slow_divmod(self, other):
    ...
    '''
    n = len(self.d)
    q = BigNum.zero(n) #quotient
    r = BigNum(self.d) #remainder
    s = [BigNum(other.d)] #s[i] = other*2^i
    i = 0
    while !s[i].d[n+1] > 0 and !s[i] > self:
      i += 1
      s[i] = s[i-1] + s[i-1]
