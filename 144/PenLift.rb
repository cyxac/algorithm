...
def num_times segments, n
  ...
  whole_graph = []
  lines.each do |l|
    head_index = whole_graph.index(l.head)
    if head_index
      head = whole_graph[head_index]
    else
      head = l.head
      whole_graph << head
    end

    tail_index = whole_graph.index(l.tail)
    if tail_index
      tail = whole_graph[tail_index]
    else
      tail = l.tail
      whole_graph << tail
    end
  ...
  # Ask for connected graps and do final calculation
  connected_graphs = []
  queue = []
  while !whole_graph.empty?
    queue << whole_graph.first
    connected_graph = []
    while !queue.empty?
      vertice = queue.shift
  ...
  connected_graphs.each do |graph|
    odd_vertices = graph.count {|v| v.connected.size % 2 == 1}
